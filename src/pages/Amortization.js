...
function Amortization() {
      ...
      for (let i = 0; i < data.length; i++) {
        if (loan[i].year > prevYear) {
          ...
          loantbl.push({
            id: loan[i].year,
            Date: loan[i].year,
            Balance: currency.format(ybalance),
            Interest: currency.format(yinterest),
            Principal: currency.format(yprincipal),
            detail: [],
          });
        }
        // console.log(i, yidx, loantbl[yidx] );
        loantbl[yidx].detail.push({
          id: i,
          Date: loan[i].year + " " + loan[i].month,
  ...
  function download(loan) {
    ...
    data.forEach((object) => {
      const rowValues = Object.values(object).map(
        (v) => `${v - Math.floor(v) !== 0 ? currency.format(v) : v}`
      );
  ...
  const handleStartDateChange = (e) => {
    setData((data) => {
      return { ...data, startDate: e.toISOString().substring(0, 10) };
    });
  };

  const handleRecurMDateChange = (e) => {
    setData((data) => {
      return { ...data, recurMDate: e.toISOString().substring(0, 10) };
    });
  };

  const handleRecurADateChange = (e) => {
    setData((data) => {
      return { ...data, recurADate: e.toISOString().substring(0, 10) };
    });
  };

  const handleOnePayDateChange = (e) => {
    setData((data) => {
      return { ...data, onePayDate: e.toISOString().substring(0, 10) };
    });
  };
      ...
      <section className="colored-section" id="title">
        <div className="container-fluid">
          ...
          <p className="main-text max-text-box">
            {" "}
            Amortization is paying off a debt over time in equal payments. A
            portion of each payment goes toward the loan principal, and another
            portion goes toward interest. As time passes, the amount going
            toward principal gradually grows larger month by month. In the
            following amortization schedule, you can see how much money you pay
            in principal and interest over time. Use this calculator to input
            the details of your loan and see how those payments break down over
            your loan term.
          </p>
              ...
              <div className="col-lg-3 col-md-1">
                <div className="loan-input">
                      ...
                      <Accordion.Body>
                        <div className="optional" id="">
                          <div className="card optional-body">
                            <p className="optional-body-msg main-text">
                              By adding extra payments, you can lower total
                              interest paid and payoff the loan faster.
                            </p>
                            ...
                            <form className="">
                              ...
                              <div className="input-group date">
                                <DatePicker
                                  className="optional-data form-input"
                                  id="recurMDate"
                                  selected={new Date(data.recurMDate)}
                                  onChange={handleRecurMDateChange}
                                />
                              </div>
                            </form>
                            <form className="">
                            ...
                            <form className="">
                              ...
                              <div className="input-group date">
                                <DatePicker
                                  className="optional-data form-input"
                                  id="recurADate"
                                  selected={new Date(data.recurADate)}
                                  onChange={handleRecurADateChange}
                                />
                              </div>
                            </form>
                            <form className="">
                            ...
                            <form className="">
                              ...
                              <div className="input-group date">
                                <DatePicker
                                  className="optional-data form-input"
                                  id="onePayDate"
                                  selected={new Date(data.onePayDate)}
                                  onChange={handleOnePayDateChange}
                                />
                              </div>
                            </form>
                          </div>
                  ...
                  <button
                    ...
                    type="button"
                    className="calcAmort-btn form-btn small-text"
                  >
              ...
              <div className="col-lg-9 col-md-3">
                <div className="card loan-output">
                  <div className="card2-header ">
                    <p className="card2-title-left tiny-text bold-text main-color">
                      SUMMARY
                    </p>
                    <p className="card2-title-right tiny-text bold-text main-color">
                      Number of Payments:
                      <span id="months">{result.months}</span>
                    </p>
                    <p className="card2-header-line"></p>
                  </div>
                  <div className="card2-body">
                    <label className="card2-body-label">Monthly Payment</label>
                    <p id="mPayment" className="card2-data">
                      {currency.format(result.mPayment)}
                    </p>
                    <label className="card2-body-label">
                      Total Interest Paid
                    </label>
                    <p id="totalInt" className="card2-data">
                      {currency.format(result.totalInt)}
                    </p>
                    <label className=" card2-body-label-2">
                      Total Cost of Loan
                    </label>
                    <p id="totalCost" className="card2-body-data-2">
                      {currency.format(result.totalCost)}
                    </p>
                    <label className=" card2-body-label-2">Payoff Date</label>
                    <p id="payOffDate" className="card2-body-data-2">
                      {result.payOffDate}
                    </p>
                    <p
                      id="savingsMsg"
                      className="small-text bold-text main-color"
                      style={{
                        display: result.intSavings ? "block" : "none",
                        color: "blue",
                      }}
                    >
                      Your additional payments will reduce your total interest
                      payment by&nbsp;
                      <b>
                        <span id="intSavings" className="subheading"></span>
                        {currency.format(result.intSavings)}
                      </b>
                      &nbsp;and will also help pay off your loan&nbsp;
                      <b>{result.payOffSavings}</b>&nbsp;months sooner.{" "}
                    </p>
                  </div>
                </div>
                    ...
                    >
                      <Tab eventKey="first" title="Chart">
                        <div className="grid-item line-chart">
                          <p className="main-text left-text bold-text main-color">
                            How payments change over the life of a{" "}
                            <span id="loan-years">{data.lterm}</span>-year loan
                          </p>
                          <p className="small-text">
                            As the term of your mortgage progresses, a larger
                            share of your payment goes toward paying down the
                            principal until the loan is paid in full at the end
                            of your term.
                          </p>
                          ...
                          <div className="chart-grid">
                            ...
                            <div className="chart-grid-second-column">
                              <div className="chart-grid-table">
                                <p className="line-heading chart-grid-table-item">
                                  As of <span id="line-date"></span>
                                </p>

                                <p className="chart-table-line"></p>
                                <p
                                  className="line-color chart-grid-table-item"
                                  style={{ backgroundColor: "#3cba9f" }}
                                >
                                  {" "}
                                </p>
                                <p className="line-label chart-grid-table-item">
                                  Interest Paid
                                </p>
                                <p
                                  className="line-data chart-grid-table-item"
                                  id="line-interest"
                                ></p>
                                <p className="chart-table-line"></p>
                                <p
                                  className="line-color chart-grid-table-item"
                                  style={{ backgroundColor: "#8e5ea2" }}
                                >
                                  {" "}
                                </p>
                                <p className="line-label chart-grid-table-item">
                                    ...
                                    <div className="container slider-section">
                                      ...
                                      <div id="csv">
                                        <button
                                          ...
                                          id="download"
                                          className="form-btn"
                                          disabled={ltbl.length ? false : true}
                      ...
                      <Tab eventKey="second" title="Schedule">
                        <div className="sched-table">
                          <p className="left-aligned-hdr main-text left-text bold-text main-color">
                            Amortization Schedule Breakdown
                          </p>
                          <p className="small-text">
                            This table lists how much principal and interest are
                            paid in each scheduled mortgage payment.
                          </p>
                          <div className="table-hdr-grid">
                            <label className="table-hdr-grid-item-label">
                              First Payment
                            </label>
                            <div
                              className="table-hdr-grid-item-data date"
                              id="firstPay"
                            >
                              {monthYear(new Date(data.startDate), 1)}
                            </div>
                            <label className="table-hdr-grid-item-label">
                              Last Payment
                            </label>
